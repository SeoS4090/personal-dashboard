...
### Tarkov (`game-tarkov`)
- tarkov.dev 기반 499개 퀘스트 그래프 — 트레이더 swim-lane, 위상 정렬 깊이, bezier 엣지, 줌, Kappa 뱃지
- **좌클릭 상세** — 그래프 우상단 **플로팅 오버레이 패널**(스크롤 무관, 항상 보임): 수행 가능 상태 뱃지 · 목표 · 보상 · 선행/후속 퀘스트(클릭 시 해당 노드로 이동) · Wiki 링크
- **우클릭** — 선행 루트 하이라이트, 선행 루트 전체 완료 버튼
- **필터 바** — 이름 검색, 트레이더 선택, `Kappa만`·`완료 숨김` 토글, 초기화
- **포커스 격리 모드** — 선택 퀘스트의 선행+후속 체인만 표시(가시성 개선), `🎯 이 경로만` / `✕ 전체 보기` 토글
- 완료 상태 저장 (`localStorage: tarkov_quest_progress`)

후보 기능 (예정):
- 플레이 기록 로그 · 게임 뉴스 피드 · 공략 북마크 · Steam 연동 · 맵/레벨 필터
